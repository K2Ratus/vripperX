...
class TaskList extends React.Component {
  ...
  getRowContent(ref) {
    ...
    const title = task.$$preloading
      ? `loading: ${task.url}`
      : task.isPreloaded
      ? task.title || _.last(task.dest.split(isWin32 ? '\\' : '/'))
      : task.url;
    ...
    return [
      ...
      >
        <div className="tasks_row_item">
          {task.uiExpand && (
            <div className="tasks_row_item_toggle" onClick={(e) => this.onToggleClick(e, task)} onDoubleClick={this.stopEvent}>
              <i className={cn('fa', { 'chevron-down': !isExpanded, 'chevron-right': isExpanded }, '-')} />
            </div>
          )}
          <div className={cn('tasks_row_item_title', { sub: task.$parent })} title={title}>
            {title}
          </div>
        </div>
        <div className="tasks_row_item">{hasProgress && <Progress task={task} />}</div>
        <div className="tasks_row_item">
          {!task.$parent && (
            <i
    ...
    return (
      <div className="tasks" onClick={this.onListClick}>
        ...
        <div className="tasks_list">
          <div className="tasks_list_content">{refs.map(this.getRowContent)}</div>
        </div>
      </div>
    );
  }
