...
class PicsList extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.task !== this.props.task || newProps.pics.length !== this.props.pics.length) {
  ...
  makePicName(pic) {
    ...
    if (pic.err) {
      const errorMsg = pic.err.err
        ? typeof pic.err.err == 'object'
          ? pic.err.err.code + (pic.err.options.url ? ': ' + pic.err.options.url : '')
          : pic.err.err
        : null;
      ...
      <div className="pics">
        {pagination && (
          <div className="pics_pagination">
            ...
            {pageL && (
              <div className="pics_pagination_arrow pics_pagination_arrow--left" onClick={this.pageLeft.bind(this)}>
                &lt;
              </div>
            )}
            {pageR && (
              <div className="pics_pagination_arrow pics_pagination_arrow--right" onClick={this.pageRight.bind(this)}>
                &gt;
              </div>
            )}
          </div>
