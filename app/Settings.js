...
class Settings extends React.Component {
  ...
  makeCheckbox(field, label, onChange) {
    onChange = onChange || ((newValue) => this.props.update(field, newValue));
    return (
      <label className="settings_check" htmlFor={field}>
        <input
          ...
          onChange={(e) => {
            e.preventDefault();
            onChange(!this.props.data[field]);
          }}
        />
    ...
    return (
      <div className="settings_root">
        ...
        <div className="settings_root_wrap">
          <input className="settings_root_val" type="text" value={this.props.data.root} readOnly />
          <button className="settings_root_set" onClick={this.changeRoot.bind(this)}>
            Change
          </button>
        </div>
      </div>
  ...
  getVersion() {
    return (
      <div className="settings_version">
        <label>
          Current version: <span id="version">v{currentVersion}</span>
        </label>
        <div className="settings_version_wrap">
          <button className="update_check" onClick={this.updateCheck.bind(this)}>
            Check for Update
          </button>
        </div>
      </div>
    ...
    return (
      <div className={cn('settings', { toggled: isToggled })}>
        <button className="settings_toggle" onClick={toggle}>
          {isToggled ? (
            <span>
              <i className="fa fa-minus-square-o" /> HIDE SETTINGS
            </span>
          ) : (
            <span>
              <i className="fa fa-plus-square-o" /> OPEN SETTINGS
            </span>
          )}
        </button>
        {isToggled && (
          <div className="settings_main">
            ...
            {!!data.autothanks && <button onClick={this.onDeleteVGCookiesCLick}>delete vipergirls cookies</button>}
            ...
            {!!data.subfolders && this.makeCheckbox('prefixSub', 'Prefix subfolders (post 7 - )')}
            {!!data.subfolders && this.makeCheckbox('suffixPost', 'Add suffix for the posts (postid)')}
