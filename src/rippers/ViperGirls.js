 ...
 * @returns {string}
 */
const makeThreadURL = (threadId) => `https://${HOSTNAME}/threads/${threadId}`;
 ...
 * @returns {string}
 */
const makeThreadPageURL = (threadId, pageId) => `${makeThreadURL(threadId)}/page${pageId}`;

/**
 * @param {(number|string)} postId
 */
const makePostURL = (postId) => `https://${HOSTNAME}/threads/?p=${postId}#post${postId}`;
...
const parseThreadPage = (html) => {
  ...
  const numberOfPages = pagination.length ? +pagination.first().text().split(' of ')[1] : 1; // 'Page 2 of 4'
...
function loadThread(threadId, pageId) {
  ...
  return qRequest({ url: makeThreadPageURL(threadId, pageId || 1) }).then((resp) => {
    ...
    if (fullThread && thread.pages > 1) {
      return Promise.all(
        _.range(2, thread.pages + 1).map((pageId) =>
          qRequest({ url: makeThreadPageURL(threadId, pageId) }).then((resp) => parseThreadPage(resp.body).posts)
        )
      ).then((allPosts) => {
