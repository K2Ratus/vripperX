...
const makeAlbumApiURL = (ownerId, albumId) => `http://api.vk.com/method/photos.get?v=5.49&owner_id=${ownerId}&album_id=${albumId}`;
...
class VKAlbumWorker extends AbstractAlbumWorker {
  ...
  static preload(task) {
    return qRequest({
      ...
      const pics = _.compact(
        resp.body.response.items.map((photo) => {
