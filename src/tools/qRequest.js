  ...
  async.retry(
    ...
    (err, resp) => {
      if (err) {
        ...
        // response was, but it was unsuccessful nature
        task.reject({
 ...
 * @returns {Promise}
 */
const qRequest = (options, priority) =>
  new Promise((resolve, reject) => {
    ...
    const host = URL.parse(options.url).hostname.replace(/^[a-z]{1,3}\d{2,3}\./i, '');
